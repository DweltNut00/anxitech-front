<template>
  <div class="aplicaciones-container">
    ...
    <v-alert type="info" variant="tonal" icon="mdi-information" class="mb-2">
      <strong>Por favor contesta las 2 encuestas:</strong> la evaluación DASS-21
      y la información complementaria son necesarias para el estudio.
    </v-alert>
    ...
    <div class="sections-grid">
      ...
      <section class="encuestas-section">
        ...
        <!-- Estado vacío -->
        <v-card
          v-else-if="aplicacionesExtra.length === 0"
          class="empty-state"
          elevation="0"
        >
          ...
          <p>
            Completa primero una evaluación DASS-21 para acceder a esta sección.
          </p>
        </v-card>
      ...
      <section class="encuestas-section">
        ...
        <!-- Estado vacío -->
        <v-card
          v-else-if="aplicaciones.length === 0"
          class="empty-state"
          elevation="0"
        >
          ...
          <p>
            Por el momento no hay encuestas activas. Te notificaremos cuando
            haya nuevas aplicaciones.
          </p>
        </v-card>
    ...
    <v-card class="info-footer" elevation="0" color="blue-grey-lighten-5">
      ...
      <div>
        <strong>Confidencialidad garantizada:</strong> Toda la información
        proporcionada es anónima y será utilizada únicamente con fines de
...
const getAplicaciones = async () => {
  ...
  try {
    ...
    dataJson = await (
      await fetch(
        import.meta.env.VITE_ENDPOINT +
          "questions.php?action=getMisAplicaciones",
        {
          method: "POST",
          headers: {
    ...
    aplicaciones.value = todasAplicaciones.filter(
      (item) => !dataJson.data.some((item2) => item.id === item2.id),
    );
  } catch (error) {
...
const getAplicacionesExtra = async () => {
  ...
  try {
    let dataJson = await (
      await fetch(
        import.meta.env.VITE_ENDPOINT +
          "questions.php?action=getAplicacionesExtra",
      )
    ).json();
    let todasAplicaciones = dataJson.data;
    ...
    dataJson = await (
      await fetch(
        import.meta.env.VITE_ENDPOINT +
          "questions.php?action=getMisAplicacionesExtra",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    aplicacionesExtra.value = todasAplicaciones.filter(
      (item) => !dataJson.data.some((item2) => item.id === item2.id),
    );
  } catch (error) {
