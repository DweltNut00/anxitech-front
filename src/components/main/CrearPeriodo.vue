        ...
        <v-window class="dialog" v-model="step">
                ...
                <v-card elevation-1 elevation="0">
                    <v-form v-model="basics_valid" class="item">
                        ...
                        <div class="field-group">
                            <div class="field">
                                <v-date-input v-model="range" label="Rango de aplicación" prepend-icon=""
                                    prepend-inner-icon="$calendar" multiple="range" :rules="basicRules"></v-date-input>
                            </div>

                            <div class="field">
                                <v-select v-model="tipo" label="Tipo de aplicación"
                                    :items="tipos" item-value="value" item-title="label" :rules="basicRules"></v-select>
                            </div>
                        </div>
                    </v-form>
            ...
            <v-dialog v-model="dialog" max-width="320" persistent>
                <v-list class="py-2" color="primary" elevation="12" rounded="lg">
                    <v-list-item prepend-icon="mdi-clock-time-four-outline" title="Registrando periodo...">
            ...
            <v-window-item :value="2">
                <v-card elevation-1 elevation="0" class="item">
                    <div class="header">
                        ...
                        <h4 class="subtitle text-grey-lighten-1">¡Excelente! Periodo registrado.
                        </h4>
                    </div>
...
const tipos = [
    {
        label: 'Encuesta de ansiedad',
        value: 0
    },
    {
        label: 'Encuesta complementaria',
        value: 1
    },
]
...
const validateRange = async () => {
    ...
    try {
        ...
        const register = await (await fetch(import.meta.env.VITE_ENDPOINT + 'questions.php?action=registerPeriodo', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                inicio: formatDate(range.value[0]),
                fin: formatDate(range.value[range.value.length - 1]),
                tipo: tipo.value
            })
        })).json()
